...
PRISM runs as a **FastAPI** application with **SQLite** state, internal BaseIntelligence authentication, and GPU evaluation through the Docker broker. The challenge owns the data and the evaluation; the validator re-executes the miner's training loop under a forced random init and computes the score itself.
...
```mermaid
flowchart LR
    Miner[Miner] --> Proxy[BaseIntelligence Proxy]
...
BaseIntelligence is responsible for miner-facing upload security. It verifies signatures, timestamps, nonces, and hotkey identity before forwarding a submission to PRISM.
...
See [PRISM API](/prism/api) for the full route list and [Running PRISM](/prism/operators) for deployment.
