...
- A registered miner **hotkey** on BaseIntelligence (netuid **100**).
...
<Steps>
  ...
  <Step title="Submit">
    Submit the bundle to the public route (when public submissions are enabled), or let the BaseIntelligence proxy forward it in production.
...
<Tip>
Make your loop deterministic under the forced seed and correct at `world_size=1`. The scored run uses one physical GPU (`nproc=1`), so a loop that only works multi-GPU will not score. See [Constraints](/prism/constraints).
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Providing context" icon="cube" href="/prism/context">
    The `PrismContext` fields your scripts receive.
  </Card>
  <Card title="Examples" icon="code" href="/prism/examples">
